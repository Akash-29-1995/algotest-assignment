...
def call() {
    withCredentials([string(credentialsId: 'do-access-token', variable: 'DO_ACCESS_TOKEN')]) {
        sh '''
            ...
            mkdir -p /tmp/kube
            export KUBECONFIG=/tmp/kube/config
            echo "export KUBECONFIG=/tmp/kube/config" >> ~/.bashrc  # persist for future steps

            doctl auth init -t $DO_ACCESS_TOKEN
            doctl kubernetes cluster kubeconfig save algotest-k8s --set-current-context=false

            echo "✅ Testing cluster access..."
            KUBECONFIG=/tmp/kube/config kubectl get nodes

            echo "✅ Deploying..."
            KUBECONFIG=/tmp/kube/config kubectl apply -f algotest-assignment/flask-app/k8s/deployment.yaml
            KUBECONFIG=/tmp/kube/config kubectl apply -f algotest-assignment/flask-app/k8s/service.yaml
            KUBECONFIG=/tmp/kube/config kubectl apply -f algotest-assignment/flask-app/k8s/ingress.yaml
        '''
    }
}